...
// Sample data for the edit form
const sampleFeeRecord = {
  id: 'RCP-001',
  studentId: 's1',
...
export default function EditFeeCollectionPage() {
  ...
  // Simulate loading the fee record
  useEffect(() => {
    if (!params?.id) return;
    ...
    const loadFeeRecord = async () => {
      ...
      try {
        ...
        setFormData({
          studentId: sampleFeeRecord.studentId,
          feeTermId: sampleFeeRecord.feeTermId,
          amount: sampleFeeRecord.amount,
          paymentMode: sampleFeeRecord.paymentMode,
          transactionReference: sampleFeeRecord.transactionReference,
          paymentDate: sampleFeeRecord.paymentDate,
          notes: sampleFeeRecord.notes,
        });
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Updating fee collection:', { id: params?.id, ...formData });
  ...
  if (isLoading) {
    return (
      <PageWrapper title="Edit Fee Payment" subtitle="Update fee payment record">
        <div className="flex justify-center items-center h-64">
          <Loader2 className="h-8 w-8 animate-spin" />
          <span className="ml-2">Loading fee record...</span>
        </div>
      </PageWrapper>
    );
  }
  ...
  return (
    <PageWrapper 
      ...
      subtitle={`Update payment record ${params?.id || ''}`}
    >
      <div className="mb-6">
        <Button
          variant="outline"
        ...
        <CardHeader>
          ...
          {selectedStudent && (
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Student: {selectedStudent.name} ({selectedStudent.admissionNo}) - {selectedStudent.class}
            </div>
          )}
        </CardHeader>
        <CardContent>
