-- Database Indexes for Recipient Query Optimization (Prisma Compatible)
-- This version can be run through Prisma CLI
...
CREATE INDEX IF NOT EXISTS idx_teacher_branch_active 
ON "Teacher" (branch_id, is_active) 
WHERE is_active = true;

-- Teacher search optimization
CREATE INDEX IF NOT EXISTS idx_teacher_search_optimized 
ON "Teacher" (branch_id, is_active, first_name, last_name, employee_code) 
WHERE is_active = true;

-- Teacher phone index
CREATE INDEX IF NOT EXISTS idx_teacher_phone 
ON "Teacher" (phone) 
WHERE phone IS NOT NULL AND phone != '';
...
CREATE INDEX IF NOT EXISTS idx_employee_search_optimized 
...
CREATE INDEX IF NOT EXISTS idx_parent_father_mobile 
ON "Parent" (father_mobile) 
WHERE father_mobile IS NOT NULL AND father_mobile != '';

CREATE INDEX IF NOT EXISTS idx_parent_mother_mobile 
ON "Parent" (mother_mobile) 
WHERE mother_mobile IS NOT NULL AND mother_mobile != '';

CREATE INDEX IF NOT EXISTS idx_parent_guardian_mobile 
...
CREATE INDEX IF NOT EXISTS idx_section_class_id 
...
CREATE INDEX IF NOT EXISTS idx_student_name_gin 
ON "Student" USING gin ((first_name || ' ' || last_name) gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_teacher_name_gin 
ON "Teacher" USING gin ((first_name || ' ' || last_name) gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_employee_name_gin 
...
CREATE INDEX IF NOT EXISTS idx_communication_message_status 
